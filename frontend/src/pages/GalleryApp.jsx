...
const CARDS_16 = [
  { href: "#/viewer?replay=replays/rl_earth_hormuz/replay.jsonl&t=6", tag: "エネルギー・シーレーン", title: "ホルムズ海峡封鎖",
    res: true, desc: <>エネルギー航路の途絶が価格と備蓄競争を伝播。<b className="res">RL世界では封鎖下で戦争1件が発生(ルールベース政策では0件)</b>。同じ介入でも政策の選択が分岐を作ります。t=6 の封鎖から。</> },
  { href: "#/viewer?replay=replays/rl_earth_hormuz_if_t5/replay.jsonl&t=5", tag: "反実仮想(IF史)", title: "IF: ホルムズを1tick早く封鎖したら",
    res: true, desc: <>同じ世界・同じRL政策で介入だけt=6→t=5に前倒し。<b className="res">エネルギー価格は2.93→2.61に収まり、戦争は発生しなかった</b> — 介入時点1tickの差が帰結を分けました。</> },
  { href: "#/viewer?replay=replays/rl_earth_financial_crisis/replay.jsonl&t=6", tag: "金融・経済", title: "世界金融危機",
    res: true, desc: <>信用収縮が貿易効率を叩き、資源輸入国から順に不足イベントが連鎖。<b className="res">平均債務は116%と、ルールベース政策(96%)より深刻化</b> — 学習ポリシーの運営が別の危険を作っています。</> },
  { href: "#/viewer?replay=replays/rl_earth_taiwan/replay.jsonl&t=6", tag: "半導体・台湾有事", title: "台湾海峡の航路遮断",
    desc: <>半導体シーレンの途絶がチップ価格を急騰させ、軍事・生産の両面に伝播します。</> },
  { href: "#/viewer?replay=replays/rl_earth_jpn_taiwan/replay.jsonl&t=6", tag: "日本の安全保障", title: "日本の周辺情勢(米ハブ同盟網)",
    desc: <>日本を米国ハブの同盟網(日米・米韓・米豪・米台)に置いた設定で台湾有事を扱う、憲章の「現在の日本における安全保障リスク」の典型シナリオ。</> },
  { href: "#/viewer?replay=replays/rl_earth_disinfo_jpn/replay.jsonl&t=5", tag: "偽情報・ギャップチャネル", title: "対日偽情報キャンペーン",
    desc: <>平時の偽情報が慢性疑心を積み上げ、不足イベントを誘発する様子。慢性的な圧力が一発の事件と別の経路で効く「ギャップチャネル」のデモです。</> },
  { href: "#/viewer?replay=replays/rl_earth_triple_crisis/replay.jsonl&t=10", tag: "複合危機", title: "三重危機",
    desc: <>エネルギー・金融・食料のショックが同時にヒットする複合シナリオ。単一危機と伝播の重なり方が変わります。</> },
];

const CARDS_ALL = [
  { href: "#/viewer?replay=replays/rl_all_hormuz/replay.jsonl&t=10", tag: "全国家・シーレーン", title: "ホルムズ海峡封鎖(176カ国)",
    res: true, desc: <>同一の介入を176カ国世界で。<b className="res">戦争6件・崩壊1</b> — 16カ国版(戦争1)より連鎖の自由度が大きく、少数精鋭の世界では見えなかった波及が出ます。</> },
  { href: "#/viewer?replay=replays/rl_all_hormuz_if_t5/replay.jsonl&t=9", tag: "全国家・反実仮想", title: "IF: 全国家世界で1tick早く封鎖(176カ国)",
    res: true, desc: <><b className="res">封鎖前倒しで戦争6→4件</b>。16カ国版と同じ分岐実験を大世界で実行 — 介入時点の感度が国家数に対してどう変わるか比較できます。</> },
  { href: "#/viewer?replay=replays/rl_all_financial_crisis/replay.jsonl&t=10", tag: "全国家・金融", title: "世界金融危機(176カ国)",
    res: true, desc: <><b className="res">債務デフォルト7件</b>。金融伝播は貿易網の密度に効くので、176カ国では脆弱国の連鎖的破綻が16カ国版より際立ちます。</> },
  { href: "#/viewer?replay=replays/rl_all_taiwan/replay.jsonl&t=10", tag: "全国家・台湾有事", title: "台湾海峡の航路遮断(176カ国)",
...
export default function GalleryApp() {
  return (
    <div className="gallery">
      <h1>Geopolitics Terrarium <small>RL政策リプレイギャラリー</small></h1>
      <p className="sub">
