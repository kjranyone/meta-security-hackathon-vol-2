...
- **拡散抑止の下では学習AIが際立って安定化する**（39.4 → 3.8件、初回開戦 t15 → t59）—
  均衡の構造を学んだ戦術層はエスカレーションを回避する。核なき世界ではその差は縮まる（219 vs 181）
- **思考AI（GLM）は初回開戦が遅い**（t4 → t6、戦争数は同等）— 協調的な外交判断が危機の起点を遅らせる（3seed・予備的観察）
...
|---|---|---|---|
| 世界GDP（開始時117兆USD＝実データ） | 124.2 | 103.0 | **-17.1%** |
| デフォルト | 14件 | **51件** | +37 |
| 平均失業率 | 5.4% | 4.7% | -0.6pt |
...
**72ヶ月長期実験**（`logs/earth_all_hormuz_72t*_v3`、閉鎖t6-24、その後48ヶ月の回復、実時間校正+実データ初期値）:

| 指標 | heuristic | 全カ国学習AI | 差分 |
|---|---|---|---|
| 世界GDP | 103.0 | **114.2** | **+10.9%** |
| デフォルト累計 | 51 | **32** | **-37%** |
| 平均失業率 | 4.7% | **4.0%** | -0.7pt |
| 平均安定度 | 53.5 | **55.7** | +2.2 |

- **全カ国学習AI化は危機対応を劇的に改善**: GDP+10.9%・デフォルト-37%・失業-0.7pt —
...
| 現状（9カ国保有） | 95.5±9.1 件 | 80.5±3.5 件 | 96.3±5.3 件 | t4 / t4（GLM t6） |
| 拡散（+IRN/KOR/SAU） | 39.4±4.5 件 | **3.8±1.8 件** | — | **t15 / t59** |
...
- **学習AIは抑止構造が強いほど安定化に寄与**（拡散で41.4 → 3.4件）。核なき世界では差は縮む（219 vs 181）—
  戦術層は「均衡の簿記」を学び、抑止の傘の下でエスカレーションを回避する。
  **構造が支配するが、AIは構造を増幅も緩和もする**
- **思考AI（z.ai GLM、3seed×72ヶ月）: 戦争 96.3±5.3 件・初回開戦 t6** — ルールAI(t4)より
