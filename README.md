...
| 拡張 | 内容 | 実測 |
|---|---|---|
| **観測** | 48→**61次元**: 政体one-hot・イデオロギー・戦争強度・難民負担(人口比)・被制裁数・選挙接近度・失業率水準・再エネ比率・自国CO2 を追加 | SAH学習: 61次元+外交なし **+6.0** |
| **ネット幅** | `--hidden 128`(既定64から倍増、3.4k→**11k params**)。npzにhidden/fine/diplomacyを記録し旧モデルと混在運用 | lh-v9: 転移GDP 120.3/安定42.6/失業5.5% |
| **外交3頭** | RLが外交(改善/制裁/脅迫)も決定する実装 — ただし**不可逆的escalationへの探索が方策学習を一貫して破壊する**ことが実証されたため既定は無効(SAH: 外交なし+6.0 vs 外交あり−3.7)。実装と知見は温存 | 失敗の記録に追加 |

### 因果ファクター拡張（v8: 5つの新しい連鎖）

| ファクター | 因果連鎖 | 実測(earth_all 72ヶ月基準) |
|---|---|---|
| **難民フロー** | 戦争強度→流出(人口0.08%/月×強度)→重心近隣3カ国が受入→受入国の安定・失業悪化→地域感染 | **114.8百万人移動**、受入は紛争隣接国に集中(タイ・ベナン・カンボジア) |
| **政体遷移(内生)** | 2年の統治危機→混合政体が権威主義化(`democratic_backsliding`)、3年の繁栄+教育→専制が民主化(`democratization`) | 政体が静的パラメータでなく内生変数に |
| **気候極端イベント** | CO2>3000で旱魃/台風ハザード(CO2に比例)→食料×0.7(6ヶ月)またはインフラ打撃 | 63件創発(CO2累積5900で) |
| **エネルギー転換** | 高油价(>1.3)+補助金→再エネ比率が漸増(上限85%)→CO2排出減。危機が長期の需給構造を変える | 1973年後の省エネ投資の類似 |
| **サイバー戦** | サイバー攻撃基盤保有国が戦時に敵のファブ生産×0.7(2ヶ月)・軍事を妨害 | 584件の`cyber_attack` |
...
- **拡散抑止の下では学習AIが際立って安定化する**（89.4 → 26.1件、初回開戦 t15 → t36）—
...
| 戦争頻度の絶対水準が高い | — | chaosシナリオは意図的極限ストレス。**比較に有効・絶対値に無効**(16カ国×6年で~100件は現実の~50倍) |
...
|---|---|---|---|
| 世界GDP（開始時117兆USD＝実データ） | 123.0 | 121.7 | **-1.1%** |
| デフォルト（外貨建て新興国に集中） | 5件 | **5件** | ±0 |
| 平均債務対GDP | 98% | **105%** | **+6pt**（危機は債務とインフレで吸収） |
...
**72ヶ月長期実験**（`logs/earth_all_hormuz_72t*_v8`=最終エンジン: 政体・戦争強度・慣性・難民・気候・サイバー・較正済み時定数、閉鎖t6-24）:
...
| 要件 | 実装 | 効果 |
|---|---|---|
| **長期戦略** | 訓練地平24→**96ヶ月**(8年) + **GAE(λ=0.95)** への置換(MC→TD偏差の混合) | 学習の安定化: 訓練評価 **610→733(+122)** 従来+11 |
| **微操作** | 予算6プリセット→**4軸×5水準**の細粒度行動(予算組合せ625通り、softmax per axis) | 未学習世界で失業 **5.4→3.3%** |
| **分布シフト耐性** | **ドメインランダム化**: earth + 生成世界3種(gen:7/13/99、`gen:<seed>`プリセット)を巡回訓練 | GRUが47カ国崩壊した16→176カ国転移で**崩壊は同等**(10 vs 9)のまま経済指標を全面改善 |
...
| **全176カ国で稼働** | ホルムズ72ヶ月: GDP 121.6兆・破綴5件・崩壊23 — ヒューリスティック版(121.7兆・5件・20)とほぼ同等 |

→ LLMの戦略判断を30KBに蒸留し、**LLMなしで全国家を動かす**ことに成功。
ただし全国家展開では崩壊がやや増える(23 vs 20) — LLM教師の「丁寧な」
予算配分が脆弱国で安定を損なう場面があり、教師データのカバレッジ限界として記録。
