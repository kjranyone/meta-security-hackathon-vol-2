...
- **抑止は強力に機能し、AI相手でも崩れない**: 核なし→現状→拡散で戦争は 219 → 99 → 41（ルールAI）に単調減少、
  **全構成で核保有国が当事者の戦争はゼロ**（抑止は「標的の選択」を変える）
- **拡散抑止の下では学習AIが際立って安定化する**（41.4 → 3.4件、初回開戦 t15 → t60）—
...
**72ヶ月長期実験**（`logs/earth_all_hormuz_72t*_v2`、閉鎖t6-24、その後48ヶ月の回復、実時間校正後）:

| 指標 | heuristic | 全カ国学習AI | 差分 |
|---|---|---|---|
| 世界GDP | 335.4 | 332.1 | -1.0% |
| デフォルト累計 | 62 | **38** | **-39%** |
| 開戦総数 | 18 | **3** | **-83%** |
| 平均失業率 | 5.78% | **4.88%** | -0.9pt |
| 平均安定度 | 45.0 | **47.5** | +2.5 |

- **全カ国学習AI化は危機対応を劇的に改善**: デフォルト-39%・開戦-83%・失業-0.9pt —
...
|---|---|---|---|---|
| 核なし | **218.8±14.9 件** | **180.5±14.2 件** | — | t4 / t4 |
| 現状（9カ国保有） | 99.3±8.7 件 | 78.3±7.6 件 | 98.0±7.5 件 | t4 / t4（GLM t6） |
| 拡散（+IRN/KOR/SAU） | 41.4±5.2 件 | **3.4±1.8 件** | — | **t15 / t60** |
...
- **思考AI（z.ai GLM、3seed×72ヶ月）: 戦争 98.0±7.5 件・初回開戦 t6** — ルールAI(t4)より
