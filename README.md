...
|---|---|---|---|
| 世界GDP | 189.2 | 151.5 | **-19.9%** |
| エネルギー価格 | 1.00 | 1.41 | **+41%** |
| 半導体価格（電力不足→ファブ減産の連鎖） | 1.00 | 1.08 | **+8%** |
| 不足イベント数 | 312 | 486 | **+56%** |

台湾海峡封鎖はGDP -12.1%（地下資源の対日輸出も台湾海峡経由）。
三重危機（ホルムズ+台湾+スエズ封鎖）では GDP **-29.9%**・デフォルト2件。
...
**創発的な洞察（seed=42, 36tick、実ログ）**: 同じホルムズ封鎖履歴を分岐させて核融合だけを禁止すると（IF史 t1）、CO2累積排出は **600 → 619 (+3.1%)** 増え、日本の債務不履行は t21→t34 に遅延する — **未来エネルギー技術の創発は気候と債務経路の両方を変える**（決定論的な因果比較）。
