...
|---|---|---|---|
| 核なし | **298.8件** | **254.4件** | — |
| 現状(9カ国) | 155.3件 | 113.9件 | 20件† |
| 拡散(+IRN/KOR/SAU) | 89.7件 | **25.7件** | — |

- **抑止は強力に機能し、AI相手でも崩れない**: 核なし→現状→拡散で戦争は 218 → 96 → 39（ルールAI）に単調減少、
  **全構成で核保有国が当事者の戦争はゼロ**（抑止は「標的の選択」を変える）
- **拡散抑止の下では学習AIが際立って安定化する**（89.7 → 25.7件、初回開戦 t15 → t36）—
...
|---|---|---|---|
| 世界GDP（開始時117兆USD＝実データ） | 123.3 | 122.0 | **-1.1%** |
...
**72ヶ月長期実験**（`logs/earth_all_hormuz_72t*_v7`=最終エンジン: 政体・戦争強度・慣性・較正済み時定数、閉鎖t6-24）:
...
| 現状（9カ国保有） | 155.3±11.3 件 | 113.9±12.9 件 | 20件† | t4 / t4（GLM **t7**） |
| 拡散（+IRN/KOR/SAU） | 89.7±15.1 件 | **25.7±7.7 件** | — | **t15 / t36** |
