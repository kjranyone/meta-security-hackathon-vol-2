...
| **安全保障ジレンマ** | 信頼<15の敵対大国の軍事優位(>1.2×)が毎tick好戦性+0.004・疑心+0.003を駆動 |
...
**主要な結果**（10 seed、24tick、混沌シナリオ固定、`analysis/out/stability_*.csv`）:

| 抑止構成 | ルールAI | 学習AI | 思考AI |
|---|---|---|---|
| 核なし | **32.6件** | **27.0件** | — |
| 現状(5カ国) | 17.2件 | 14.5件 | 2.5件 |
| 拡散(+IRN/KOR/SAU) | **0.0件** | **0.0件** | — |

- **抑止は「頻度」でなく「標的の選択」を変える**（初回開戦は全構成でt3、しかし核保有構成では核保有国が絡む戦争が発生しない）
- **戦術AI層は危機安定性を変えない**（8.5 vs 8.6）— 構造（抑止構成）が支配する
- **思考AI（GLM）政府はむしろ緩和的**（初回開戦t8、戦争2.5件、予備的）

→ **意思決定AIのリスク議論は「AIの性質」単独でなく「構造との相互作用」に焦点を当てるべき**
という分離を実証した。詳細は [危機安定性実験](#危機安定性実験q1q2-ai意思決定者と抑止の相互作用)。
...
三重危機（ホルムズ+台湾+スエズ封鎖）では GDP **-29.3%**（130.2）・デフォルト2件。
...
**72tick長期実験**（`logs/earth_all_hormuz_72t*`、閉鎖t6-24、その後48ヶ月の回復）:

| 指標 | mock | RL |
|---|---|---|
| 世界GDP | 4111.8 | **4135.5** (+0.6%) |
| デフォルト累計 | 70 | **65** (-7%) |
| 戦争開始 | 27 | 29 |
| 平均失業率 | 6.5% | **6.0%** |

- **t57以降に戦争の波**: イラン系小国(IRB)が近隣と連鎖的に衝突 → **安全保障ジレンマの長期発火**
- **因子の創発拡大**: 通貨ブロック94カ国・輸出規制62カ国・核9カ国（恐慌後に規模拡大）
- **5カ国の政府崩壊**、13の技術創発と2,209回の技術採用が記録される
- RL版はデフォルト7%減・失業0.5pt低いが、戦争は同等 → 戦術層の貢献は経済回復で、
  エスカレーション抑制には効かない（Q1の結論と整合）
...
- **核の有無がエスカレーションを桁で変える**（核なしの 1.9 倍 → 拡散で完全に消失。相互防衛と安全保障ジレンマ導入後は全構成で戦争が増加）
- **初回開戦は全構成で t3-4**（核保有構成でも初期の紛争は同じ速さで始まるが、
  核保有国との大規模戦争は発生しない = 抑止は「頻度」ではなく「標的の選択」を変える）
- ルールAIと学習AIの差は小さい（17.2 vs 14.5、ジレンマ力学でRL側がやや低い）= 戦術層のAI化は危機安定性を大きくは変えない、
