...
例（実世界プリセット、ホルムズ海峡封鎖、seed=42, 36ヶ月、`server/logs/` の実ログ）:

| 指標 | baseline | 封鎖シナリオ | 差分 |
|---|---|---|---|
| 世界GDP | 189.2 | 154.2 | **-18.5%** |
| エネルギー価格 | 1.00 | 1.22 | **+22%** |
| 半導体価格（電力不足→ファブ減産の連鎖） | 1.00 | 1.09 | **+9%** |
| 不足イベント数 | 312 | 457 | **+46%** |

台湾海峡封鎖はGDP -11.9%（地下資源の対日輸出も台湾海峡経由）。
三重危機（ホルムズ+台湾+スエズ封鎖）では GDP **-29.8%**・デフォルト2件。
...
ゲームライクな表示:
- **地図上部に西暦カレンダー**（西暦2026年8月開始、1tick=1ヶ月。日・時刻は演出として再生中に進む）
...
**創発的な洞察（seed=42, 36ヶ月、実ログ）**: ホルムズ封鎖のエネルギー価格は1.22にとどまる（t20以降に核融合・宇宙太陽光が普及し、**未来技術が海峡ショックを吸収**する）。一方 `earth_ban_fusion`シナリオで神が核融合を禁じてから封鎖するとエネルギー価格は**1.34**まで跳ね上がる — **未来エネルギー技術の禁止は海峡依存を固定化する**。未来予測と地政学の相互作用そのものを観測できる。
