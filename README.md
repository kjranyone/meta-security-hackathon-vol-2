...
|---|---|---|---|
| 世界GDP | 188.2 | 151.8 | **-19.3%** |
| エネルギー価格 | 1.00 | 1.31 | **+31%** |
| 半導体価格（電力不足→ファブ減産の連鎖） | 1.00 | 1.08 | **+8%** |
| 不足イベント数 | 312 | 477 | **+53%** |

台湾海峡封鎖はGDP -12.1%（165.5）（地下資源の対日輸出も台湾海峡経由）。
三重危機（ホルムズ+台湾+スエズ封鎖）では GDP **-29.9%**（131.9）・デフォルト2件。
...
第2因子として**輸出規制レジーム**（`export_control`）を実装済み: 加盟は4tick、
**加盟国の制裁は自動的にレジーム全体へ伝播**（`collective_sanction` イベント、
元の制裁をparentに持つ）。heuristicは「制裁されている/大国/戦時」に加盟を表明する。

新しい因子（通貨ブロック、核傘…）は **CATALOG に FactorSpec を1つ足すだけ**。
