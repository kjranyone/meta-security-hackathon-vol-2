...
**結果（10 seed、24tick、earth_chaos 固定、earthプリセット）**:

| 抑止構成 | ルールAI | 学習AI |
|---|---|---|
| 核なし | **23.4±5.6 件** | **20.7±2.9 件** |
| 現状（5カ国保有） | 8.5±1.6 件 | 8.6±1.6 件 |
| 拡散（+IRN/KOR/SAU） | **0.0±0.0 件** | **0.0±0.0 件** |

- **核の有無がエスカレーションを桁で変える**（核なしの 2.7 倍 → 拡散で完全に消失）
- **初回開戦は全構成で t3-4**（核保有構成でも初期の紛争は同じ速さで始まるが、
  核保有国との大規模戦争は発生しない = 抑止は「頻度」ではなく「標的の選択」を変える）
- ルールAIと学習AIの差は小さい（8.5 vs 8.6）= 戦術層のAI化は危機安定性を変えない、
  **構造（抑止構成）が支配する** → 意思決定AIのリスク議論は「構造との相互作用」に焦点を当てるべき
- **思考AI（z.ai GLM、12tick×2run）: 戦争 2.5±0.5 件・初回開戦 t8** — ルールAI(t3)より
  開戦が**遅く少ない**（協調的な外交判断がエスカレーションを緩和）。ただし12tick止まりなので
  予備的観察として扱う（LLMは非決定論で高コスト、10seed×24tickは現実的でない）
