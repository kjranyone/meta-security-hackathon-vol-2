...
|---|---|---|---|
| 核なし | **219.6件** | **161.3件** | — |
| 現状(9カ国) | 100.2件 | 75.1件 | 108.0件 |
| 拡散(+IRN/KOR/SAU) | 49.3件 | **6.6件** | — |

- **抑止は強力に機能し、AI相手でも崩れない**: 核なし→現状→拡散で戦争は 218 → 96 → 39（ルールAI）に単調減少、
  **全構成で核保有国が当事者の戦争はゼロ**（抑止は「標的の選択」を変える）
- **拡散抑止の下では学習AIが際立って安定化する**（49.3 → 6.6件、初回開戦 t15 → t58）—
...
|---|---|---|---|
| 世界GDP（開始時117兆USD＝実データ） | 123.0 | 100.2 | **-18.5%** |
| デフォルト | 14件 | **55件** | +41 |
| 平均失業率 | 5.0% | 5.0% | ±0 |
...
**72ヶ月長期実験**（`logs/earth_all_hormuz_72t*_v4`、閉鎖t6-24、その後48ヶ月の回復、実時間校正+実データ+思想）:

| 指標 | heuristic | 全カ国学習AI | 差分 |
|---|---|---|---|
| 世界GDP | 100.2 | **111.1** | **+10.9%** |
| デフォルト累計 | 55 | **38** | **-31%** |
| 平均失業率 | 5.0% | **4.7%** | -0.3pt |
| 平均安定度 | 52.1 | **53.0** | +0.9 |

- **全カ国学習AI化は危機対応を劇的に改善**: GDP+10.9%・デフォルト-31%・失業-0.3pt —
  generalist戦術AIは封鎖下の債務規律とエスカレーション回避を学習している
...
| 現状（9カ国保有） | 100.2±11.2 件 | 75.1±3.2 件 | 108.0±8.8 件 | t4 / t4（GLM t5） |
| 拡散（+IRN/KOR/SAU） | 49.3±8.6 件 | **6.6±2.1 件** | — | **t15 / t58** |
...
- **思考AI（z.ai GLM、3seed×72ヶ月）: 戦争 108.0±8.8 件・初回開戦 t5** — ルールAI(t4)より
...
**金融感染の実測**（`earth_financial_crisis` = ホルムズ封鎖＋利上げ8%、seed=42、実時間校正後）:
```
t6  日本 が債務不履行（利率20.6%）        ← 封鎖→輸入インフレ→金利上昇
t8  エジプト → t9 米国 → t10 インド → t11 中国 → t12 トルコ → t13 EU
    （日本債のcredibility_hitから連鎖的に感染）
t19 日本 再び債務不履行 → t22 米国 再び → t32-35 第2波（インド・エジプト・EU）
```
デフォルト計12件・世界GDP **-18.6%**（baseline比）・平均失業率13.9%。IF史でt4に日本を救済すると
感染は**数ヶ月遅れるが止まらない**（エジプトt8→t12、中国t11→t13）— 単一国救済では
連鎖の停止に十分でないことが反実仮想で示せる。
